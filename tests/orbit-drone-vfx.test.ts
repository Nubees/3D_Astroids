...
describe('Phase 7i-2 — createDroneBeam', () => {
  it('returns a Mesh with MeshBasicMaterial + AdditiveBlending + red color', () => {
    // Phase 7i-2 hotfix #10 — plasma shader + glow layer reverted per
    // user "Drop the shader, go solid + 2× wider". createDroneBeam is
    // back to a single MeshBasicMaterial cylinder (no dispatcher, no
    // glow). The cylinder r=0.48 (was 0.24 in hotfix #7, doubled again
    // per hotfix #10) — at camera z=20, FOV=60° that's a 0.96u-diameter
    // beam = ~50px on a 1280px viewport. Bloom is DISABLED in this
    // project (see src/post-processing.ts:23-36) so the wider cylinder
    // is the only lever for visual area. Saturated red 0xff0033 reads
    // as a confident wide red laser without the color washout the
    // shader's outer-color mix caused.
