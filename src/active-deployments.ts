    ...
    for (let i = 0; i < dep.perDrone.length; i++) {
      ...
      if (drone.beamLine && drone.beamLine.visible) {
        ...
        if (drone.beamAge >= ORBIT_DRONES_BEAM_LIFETIME_SECONDS) {
          ...
          //
          // Phase 7i-2 hotfix #10 — beam reverted to a single
          // MeshBasicMaterial cylinder (no shader, no glow). The
          // updateBeam helper handles the pose (drone→target midpoint,
          // Y-scale to length, quaternion to align +Y with direction).
...
export function disposeDroneDeployment(dep: DroneDeploymentState, scene: Object3D): void {
  for (const drone of dep.perDrone) {
    ...
    if (drone.beamLine) {
      ...
      //
      // Phase 7i-2 hotfix #10 — beam reverted to a single
      // MeshBasicMaterial cylinder (no shader, no glow). The Material
      // base class dispose() handles the single material.
